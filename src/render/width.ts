...
// Zero-width / formatting characters that occupy no cell (zero-width space,
// joiners, BOM). Treated as width 0 so they can mark soft wrap breaks without
// affecting visual width.
function isZeroWidthCodePoint(codePoint: number): boolean {
  return (codePoint >= 0x200B && codePoint <= 0x200F) || codePoint === 0xFEFF;
}
