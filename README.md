...
A Claude Code statusline plugin with **first-class GLM & DeepSeek provider support** — shows context health, account balance & weekly token usage, active tools, running agents, and todo progress. Always visible below your input.

Fork of [claude-hud](https://github.com/jarrodwatts/claude-hud), extended with native usage tracking for third-party Claude-compatible providers (Z.ai / bigmodel.cn GLM, DeepSeek).
...
On Windows, Node.js LTS is the supported runtime for Claude Statusline setup. If setup says no JavaScript runtime was found, install Node.js for your shell first:
```powershell
winget install OpenJS.NodeJS.LTS
```
Then restart your shell and run `/claude-statusline:setup` again.
...
Claude Statusline gives you better insights into what's happening in your Claude Code session.

---

## ✨ GLM & DeepSeek Provider Support

Beyond the standard Anthropic usage limits, this fork adds **native usage tracking for GLM and DeepSeek** — the main reason to use this fork over upstream. Providers are **auto-detected** from `ANTHROPIC_BASE_URL`; no extra config flag is needed.

| Provider | What you see | Detection |
|----------|--------------|----------|
| **GLM** (Z.ai / bigmodel.cn) | 5h / 7d quota windows, weekly token count, semantic window types (rolling/cycle) | `ANTHROPIC_BASE_URL` contains GLM domain |
| **DeepSeek** | Account balance (`¥`/`$`), natural-week token count, estimated session cost | `ANTHROPIC_BASE_URL` contains DeepSeek endpoint |
| **Anthropic** (default) | 5h / 7d subscriber rate limits from stdin | No `ANTHROPIC_BASE_URL` |

### Setup for GLM / DeepSeek

Point Claude Code at your provider, and the statusline picks it up automatically:
...
- **GLM**: 5h window with cycle/rolling semantics, 7d as token count (`7d: 138M, 5d 3h`) when no unit:6 quota limit. Limit-reached detection only fires on 5h===100% (7d is an estimate).
- **DeepSeek**: `$sessionCost/$balance · 7d:weeklyTokens`. Session cost uses provider-maintained pricing (CNY balance converts USD cost at ×7). 5-minute balance cache.
...
### Optional lines (enable via `/claude-statusline:configure`)
```
...
Claude Statusline uses Claude Code's native **statusline API** — no separate window, no tmux required, works in any terminal.

```
Claude Code → stdin JSON → claude-statusline → stdout → displayed in your terminal
...
Edit `~/.claude/plugins/claude-statusline/config.json` directly for advanced settings such as `colors.*`,
`pathLevels`, `maxWidth`, threshold overrides, `display.timeFormat`, and `display.promptCacheTtlSeconds`. Running `/claude-statusline:configure`
preserves those manual settings while still letting you change `language`, layout, and the common
guided toggles.

Chinese HUD labels are available as an explicit opt-in. English stays the default unless you choose `中文` in `/claude-statusline:configure` or set `language` in config. The short `zh` alias remains valid, and new guided config writes the canonical `zh-Hans` value.
...
| `display.externalUsageWritePath` | string | `""` | Optional absolute `.json` path in an existing directory. When stdin `rate_limits` exists, ClaudeStatusline writes a private snapshot for other local tools. Relative paths, non-json files, and missing parent directories are ignored |
...
`display.showCost` is fully opt-in. ClaudeStatusline prefers the native `cost.total_cost_usd` field that Claude Code provides on stdin when it is available. If that field is absent or invalid for a direct Anthropic session, ClaudeStatusline falls back to the existing local transcript-based estimate so the cost line still works on older payloads. The native field is absent before the first API response in a session, so the cost display may stay hidden until then. ClaudeStatusline also keeps the cost hidden for known routed providers such as Bedrock and Vertex AI, because cloud-provider billed sessions may report `$0.00` or omit the field even though the session was not literally free.

`display.showPromptCache` is fully opt-in. When enabled, ClaudeStatusline looks at the timestamp of the last assistant response in the local transcript and shows a live countdown until the prompt cache expires. The default TTL is 5 minutes (`300` seconds). Set `display.promptCacheTtlSeconds` to `3600` if you want a 1-hour Max-style window. If the transcript does not have an assistant timestamp yet, the cache element stays hidden.
...
ClaudeStatusline prefers the official statusline stdin payload for rate-limit windows. If `display.externalUsagePath` points to a fresh local sidecar snapshot, ClaudeStatusline can append its `balance_label` alongside stdin windows. If stdin `rate_limits` are missing, the same snapshot can provide fallback usage windows.
...
Set `display.externalUsageWritePath` if you want ClaudeStatusline to write the official stdin `rate_limits` into a local snapshot for other tools. The path must be absolute, end in `.json`, and live in an existing directory. ClaudeStatusline writes the file with private permissions and ignores invalid paths quietly.
...
manual-only today; `/claude-statusline:configure` preserves it without editing it.
...
- If you configured `display.externalUsagePath`, ClaudeStatusline will try that local snapshot before hiding usage
- ClaudeStatusline never falls back to credential scraping or undocumented API calls
...
Set the `CLAUDE_STATUSLINE_DISABLE` environment variable to launch a session without the HUD — no need to remove the `statusLine` entry from `settings.json`. The legacy `CLAUDE_HUD_DISABLE` name is still honoured for backward compatibility (the new name takes precedence when both are set):

```bash
CLAUDE_STATUSLINE_DISABLE=1 claude
```
...
- Delete config and run `/claude-statusline:configure` to regenerate
...
- Make sure `CLAUDE_STATUSLINE_DISABLE` is not set in your environment (e.g. exported from a shell profile) — it silences the HUD entirely, including setup verification
...
```bash
git clone https://github.com/fyeeme/claude-statusline
...
[![Star History Chart](https://api.star-history.com/svg?repos=fyeeme/claude-statusline&type=Date)](https://star-history.com/#fyeeme/claude-statusline&Date)
