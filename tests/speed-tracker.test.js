...
async function createTempHome() {
  return await mkdtemp(path.join(tmpdir(), 'claude-statusline-speed-'));
}
...
test('getOutputSpeed returns null when output tokens are missing', () => {
  const speed = getOutputSpeed({
    transcript_path: '/tmp/claude-statusline-speed-missing.jsonl',
...
test('getOutputSpeed creates fallback cache under CLAUDE_CONFIG_DIR by default', async () => {
  ...
  try {
    ...
    const customCacheDir = path.join(customConfigDir, 'plugins', 'claude-statusline', 'speed-cache');
    const defaultCacheDir = path.join(tempHome, '.claude', 'plugins', 'claude-statusline', 'speed-cache');
...
test('getOutputSpeed ignores fallback tracking for non-file transcript paths', async () => {
  ...
  try {
    ...
    const cacheDir = path.join(tempHome, '.claude', 'plugins', 'claude-statusline', 'speed-cache');
