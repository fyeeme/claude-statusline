...
> ⚠️ **本文档部分内容已过时**：下方描述的本地缓存（`.usage-cache.json`）、两阶段 TTL、5h 轻量刷新、错误缓存、EMA 校准与单调递增保障等机制**均已从代码中移除**。当前实现：每次状态栏刷新直接请求 GLM API（`fetchFull`），无跨调用缓存；错误统一返回 `null`。因此「缓存机制」「TTL」「错误处理中的缓存策略」「缓存未命中时的完整流程」「`cache=HIT` 日志」等章节与当前实现不符，待整体重写。
