# 发布 claude-hud 到 Claude Code Marketplace 指南
...
在 GitHub 创建一个新的公开仓库（如 `linzikg/claude-hud-marketplace`），根目录放一个 `marketplace.json`：

```json
{
  "name": "linzikg-marketplace",
  "owner": {
    "name": "linzikg",
    "email": "your@email.com"
  },
  "metadata": {
    "description": "Claude Code plugins by linzikg",
    "version": "1.0.0"
  },
  "plugins": [
    {
...
```bash
# 步骤 1: 添加 Marketplace
/plugin marketplace add linzikg/claude-hud-marketplace

# 步骤 2: 安装插件
/plugin install claude-hud@linzikg-marketplace
...
```json
{
  "name": "claude-hud",                    // 必须，全局唯一
  "version": "0.0.13",                     // 建议每次发版更新
  "description": "...",                    // 建议包含
  "author": { "name": "...", "url": "..." },
  "license": "MIT",
  "repository": "https://github.com/linzikg/claude-hud",
  "homepage": "https://github.com/linzikg/claude-hud",
...
用户执行 `/plugin update claude-hud` 时，Claude Code 会：

1. 从 marketplace source 拉取最新代码
2. 复制到 `~/.claude/plugins/cache/claude-hud/claude-hud/<version>/`
3. 重新加载插件
...
当前 claude-hud 使用 `~/.claude/plugins/claude-hud/` 存放缓存，已兼容此机制。
...
# 1. Fork 或创建 claude-hud 仓库（如果还没有）
git remote add origin git@github.com:linzikg/claude-hud.git

# 2. 创建 marketplace 仓库
# 在 GitHub 上创建 linzikg/claude-hud-marketplace
git clone git@github.com:linzikg/claude-hud-marketplace.git /tmp/marketplace
cd /tmp/marketplace
...
cat > marketplace.json << 'EOF'
{
  ...
  "plugins": [
    {
      "name": "claude-hud",
      "source": { "source": "github", "repo": "linzikg/claude-hud", "ref": "v0.0.13" },
...
git add . && git commit -m "Add claude-hud plugin" && git push

# === 每次发版 ===

# 4. 在 claude-hud 仓库中更新版本、构建、测试
cd /path/to/claude-hud
...
git commit -am "Update claude-hud to 0.0.13" && git push
